...
class BulkImageUploader:
    ...
    def setup_ui(self):
        ...
        # Configuration Section (Client ID + Threads only - API settings are fixed)
        config_frame = ttk.LabelFrame(main_frame, text="Configuration", padding="15")
        config_frame.grid(row=1, column=0, columnspan=5, sticky=(tk.W, tk.E), pady=(0, 15))
        
        # Single Row: Client ID + Validate + Status + Threads
        client_id_row = ttk.Frame(config_frame)
        client_id_row.grid(row=0, column=0, sticky=(tk.W, tk.E))
        
        ttk.Label(client_id_row, text="Client ID:", font=("Segoe UI", 9, "bold")).pack(side=tk.LEFT, padx=(0, 8))
        self.client_id_entry = ttk.Entry(client_id_row, textvariable=self.client_id, width=25, font=("Segoe UI", 9))
        self.client_id_entry.pack(side=tk.LEFT, padx=(0, 8))
        ...
        self.client_id_status_label.pack(side=tk.LEFT, padx=(0, 15))
        
        ttk.Label(client_id_row, text="Threads:", font=("Segoe UI", 9)).pack(side=tk.LEFT, padx=(0, 5))
        ...
        file_frame = ttk.LabelFrame(main_frame, text="Input File Selection", padding="20")
        file_frame.grid(row=2, column=0, columnspan=5, sticky=(tk.W, tk.E), pady=(0, 10))
        ...
        progress_frame.grid(row=5, column=0, columnspan=5, sticky=(tk.W, tk.E), pady=(0, 15))
        ...
        # Summary Section
        summary_frame = ttk.LabelFrame(main_frame, text="Upload Summary", padding="20")
        summary_frame.grid(row=6, column=0, columnspan=5, sticky=(tk.W, tk.E), pady=(0, 15))
        ...
        self.summary_label = tk.Label(
            summary_content,
            text="Total: 0 | ✓ Successful: 0 | ✗ Failed: 0",
        ...
        log_frame.grid(row=7, column=0, columnspan=5, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 15))
        ...
        footer_frame.grid(row=8, column=0, columnspan=5, sticky=(tk.W, tk.E), pady=(10, 0))
        ...
        main_frame.rowconfigure(7, weight=1)  # Make log section expandable
    ...
    def process_upload(self):
        ...
        try:
            ...
            # Create output dataframe with only serial_number and image_path columns
            output_df = pd.DataFrame()
            output_df['serial_number'] = df['serial_number']
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    # Update output DataFrame (only serial_number and image_path columns)
                    if result['success']:
                        output_df.at[index, 'image_path'] = result['url']
                        self.success_count += 1
                    else:
                        output_df.at[index, 'image_path'] = 'FAILED'
                        self.fail_count += 1
            ...
            if self.is_uploading:
                ...
                self.log_message(f"Upload process completed: {total_count} total, {self.success_count} succeeded, {self.fail_count} failed", "INFO")
                self.log_message(f"✓ Successful uploads: {self.success_count}", "SUCCESS")
                self.log_message(f"✗ Failed uploads: {self.fail_count}", "ERROR")
                
                # Update summary section
                self.root.after(0, self.update_final_summary)
                
                # Save updated file (only serial_number and image_path columns)
                self.save_updated_file(output_df)
    ...
    def compress_image(self, image_path, max_size_mb=2, quality=85, min_quality=65):
        ...
        # Skip compression if file is already less than 2MB
        if orig_size is not None and orig_size < max_bytes:
            self.log_message(f"Skipping compression: {os.path.basename(image_path)} is only {orig_size / (1024*1024):.2f}MB", "DEBUG")
            return image_path
    ...
    def update_status_counts(self):
        ...
        self.summary_label.config(
            text=f"Total: {total} | ✓ Successful: {self.success_count} | ✗ Failed: {self.fail_count}"
        )
    
    def update_final_summary(self):
        """Update summary section with final counts"""
        total = self.success_count + self.fail_count
        summary_text = f"Total: {total} | ✓ Successful: {self.success_count} | ✗ Failed: {self.fail_count}"
